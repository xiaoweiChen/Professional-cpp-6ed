...
当 SpreadsheetCell 对象在操作符的左侧时，隐式转换工作正常，但当它在右侧时则不工作。加法应该是交换的呀！这里的问题是 operator+ 成员函数必须在 SpreadsheetCell 对象上使用，并且该对象必须在操作符的左侧，这就是 C++ 语言的定义方式。因此，无法使此代码使用 operator+ 成员函数工作。
