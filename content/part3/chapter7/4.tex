...
输出清楚地显示了在哪个文件和哪一行内存分配但从未释放。行号在文件名后面的括号中立即出现。花括号之间的数字是对程序启动以来内存分配的计数器。例如，\{147\} 表示程序自启动以来的第 147 次分配。可以使用 VC++ \_CrtSetBreakAlloc() 函数告诉 VC++ 调试运行时在执行特定分配时中断调试器。例如，可以在 main() 函数的开头添加以下行，指示调试器在执行第 147 次分配时中断：
