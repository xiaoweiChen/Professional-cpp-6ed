...
第二个 push\_back() 调用触发了 vector 自身的大小调整，为第二个元素分配空间。这个调整导致移动构造函数被调用，将每个元素从旧 vector 移动到新的调整大小后的 vector 中，[4]。将调用复制构造函数，将 mc 第二次复制到 vector 中，[3]。移动和复制的顺序未定义，所以 [3] 和 [4] 可能会颠倒。
