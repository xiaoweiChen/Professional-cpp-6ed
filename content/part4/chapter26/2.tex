第12章中展示的 const char* 类型的 Grid 类模板特化称为全特化模板，因为它特化了 Grid 模板的所有模板参数，特化中不再有模板参数。这并不是唯一可以特化类的方式；还可以编写部分类模板特化，在其中特化一些模板参数而不特化其他参数。例如，回想一下带有宽度（width）和高度（height）非类型参数的 Grid 模板的基本版本：
