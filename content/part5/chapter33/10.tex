...
接下来，Application、Window 和 Shape 类都是从 Handler 类派生的具体处理者。这个例子中，Application 只处理 RightMouseButtonDown 消息，Window 只处理 LeftMouseButtonUp 消息，而 Shape 只处理 LeftMouseButtonDown 消息。如果处理者收到它不认识的消息，会调用链中的下一个处理者。
